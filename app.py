...
if not GEMINI_KEYS:
    logger.warning("No Gemini API keys found. Please set them in your environment before making requests.")
    # Initialize with a dummy key to prevent crashes in langchain if it strictly requires one
    GEMINI_KEYS = ["dummy_key"]
...
class LLMWithFallback:
    ...
    def bind_tools(self, tools):
        ...
        if not llms:
            llms.append(ChatGoogleGenerativeAI(model=self.models[0], temperature=self.temperature, google_api_key="dummy").bind_tools(tools))
...
async def check_llm_keys_models():
    ...
    for model in (_MODEL_HIERARCHY or ["gemini-3.5-pro"]):
